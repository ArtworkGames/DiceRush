...
namespace StepanoffGames.DiceRush.Gameplay
{
	public class Deck : MonoBehaviour
	{
		...
		public async UniTask<int> ConfirmDiceRoll(Player player, int diceValue)
		{
			_selectedCard = null;
			...
			if (_selectedCard != null)
			{
				...
				diceValue = await _selectedCard.UseForDice(diceValue);
		...
		public async UniTask<CellType> ConfirmTileTake(Player player, CellType tileType, bool hasNearMoveBackwardCell)
		{
			_selectedCard = null;
			_confirmSelected = false;
			...
			if (_selectedCard != null)
			{
				...
				await _selectedCard.ShowSelected();
				tileType = await _selectedCard.UseForTile(tileType, hasNearMoveBackwardCell);
				await _selectedCard.HideSelected();
		...
		private async UniTask ShowCards(Player player, CardKind kind)
		{
		...
		private async UniTask AddCard(CardModel cardModel)
		{
			...
			string cardPath = $"Gameplay/UI/Deck/{cardName}.prefab";
