...
namespace StepanoffGames.DiceRush.UI.Deck
{
	public class DeckPanel : MonoBehaviour
	{
		...
		public async UniTask<CardModel> SelectCard(List<CardModel> cardModels, int totalCardsCount, CancellationToken ct)
		{
			_selectedCard = null;
			_confirmSelected = false;

			ShowCards(cardModels);
			await UniTask.WaitUntil(() => IsAllCardsShown(), cancellationToken: ct);

			EnableCards();
			await _confirmButton.Show(ct);

			await UniTask.WaitUntil(() => _selectedCard != null || _confirmSelected, cancellationToken: ct);

			_confirmButton.Hide(ct).Forget();
			HideUnselectedCards();
			...
			{
				await _selectedCard.ShowSelected(ct);
				_selectedCard.HideSelected(ct).Forget();
