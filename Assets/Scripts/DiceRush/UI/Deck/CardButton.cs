...
namespace StepanoffGames.DiceRush.UI.Deck
{
	public class CardButton : TweenButton
	{
		...
		private async UniTask LoadCard(CancellationToken ct)
		{
		...
		public async UniTask ShowSelected(CancellationToken ct)
		{
			_canvasGroup.interactable = false;
			_canvasGroup.blocksRaycasts = false;
			...
			await UniTask.WaitUntil(() => tweenCompleted, cancellationToken: ct);
		}

		public async UniTask HideSelected(CancellationToken ct)
		{
			bool tweenCompleted = false;
